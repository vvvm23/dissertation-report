...
\begin{align}
\begin{split}
    \mathbb{L}_\text{PIX} &= \alpha_\text{PIX} \cdot |\image - \hat{\image}| \cdot \\
    \mathbb{L}_\text{VQ} &= \alpha_\text{VQ} \cdot \left(||\hat{\latent} -
    \latent||^2 + ||sg[\vqganEncoder(x)] - \latent||^2_2 + ||\vqganEncoder(x) -
    sg[\latent]||^2_2\right)\\
    \mathbb{L}_\text{GAN} &= \alpha_\text{GAN} \cdot \left(\log D(x) + \log
    (1-D(\hat{x}))\right) \\
    \lambda &= \frac{\nabla_{G_{-1}}[\mathbb{L}_\text{PIX} +
    \mathbb{L}_\text{PER}]}{\nabla_{G_{-1}}[\mathbb{L}_\text{GAN}] + \epsilon}\\
    \mathbb{L} &= \mathbb{L}_\text{VQ} + \lambda \cdot \mathbb{L}_\text{GAN}\\
...
$\mathbb{L}_\text{PER}$ implemented using a pretrained VGG16
...
samples. Improving the \gls{vq} image model itself is not the focus of this
research project.
