...
\begin{align}
\begin{split}
    \mathcal{L}_\text{PIX} &= \alpha_\text{PIX} \cdot |\image - \hat{\image}| \cdot \\
    \mathcal{L}_\text{VQ} &= \alpha_\text{VQ} \cdot \left(||\hat{\latent} -
    \latent||^2 + ||sg[\vqganEncoder(x)] - \latent||^2_2 + ||\vqganEncoder(x) -
    sg[\latent]||^2_2\right)\\
    \mathcal{L}_\text{GAN} &= \alpha_\text{GAN} \cdot \left(\log D(x) + \log
    (1-D(\hat{x}))\right) \\
    \lambda &= \frac{\nabla_{G_{-1}}[\mathcal{L}_\text{PIX} +
    \mathcal{L}_\text{PER}]}{\nabla_{G_{-1}}[\mathcal{L}_\text{GAN}] + \epsilon}\\
    \mathcal{L} &= \mathcal{L}_\text{VQ} + \lambda \cdot \mathcal{L}_\text{GAN}\\
...
implemented using a pretrained VGG16 model~\cite{karen2014vg18}, and
$\nabla_{G_{-1}}[\cdot]$ is the gradient with respect to the last layer of the
\gls{vqgan} decoder.

This means there is less weighting on
