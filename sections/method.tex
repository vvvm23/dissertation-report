...
so the training our own \gls{vqgan} was necessary (see \S\ref{subsec:megagan}). 
...
also apply to \gls{vq} image modelling. 
...
\gls{nar} solutions~\cite{bondtaylor2021unleashing}, one clear
advantage of \gls{nar} models is that they are not limited to causal
...
inpainting tasks that are more complex to implement with \gls{ar} models.
